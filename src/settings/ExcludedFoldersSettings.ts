...
export async function renderExcludeFolders(settingsTab: SettingsTab): Promise<void> {
	...
	const manageExcluded = new Setting(containerEl)
		...
		.setName(tr('Manage excluded folders'));
	const desc3 = document.createDocumentFragment();
	desc3.append(
		tr('Add {regex} at the beginning of the folder name to use a regex pattern.'),
		desc3.createEl('br'),
		tr('Use * before and after to exclude folders that include the name between the *s.'),
		desc3.createEl('br'),
		tr('Use * before the folder name to exclude folders that end with the folder name.'),
		desc3.createEl('br'),
		tr('Use * after the folder name to exclude folders that start with the folder name.'),
	);
	manageExcluded.setDesc(desc3);
	manageExcluded.infoEl.appendText(tr('The regexes and wildcards are only for the folder name, not the path.'));
	manageExcluded.infoEl.createEl('br');
	manageExcluded.infoEl.appendText(tr('If you want to switch to a folder path delete the pattern first.'));
	manageExcluded.infoEl.style.color = settingsTab.app.vault.getConfig('accentColor') as string || '#7d5bed';

	new Setting(containerEl)
		.setName(tr('Whitelisted folders'))
		.setDesc(tr('Folders that override the excluded folders/patterns'))
		.addButton((cb) => {
			cb.setButtonText(tr('Manage'));
			cb.setCta();
			cb.onClick(async () => {
	...
	new Setting(containerEl)
		.setName(tr('Exclude folder default settings'))
		.addButton((cb) => {
			cb.setButtonText(tr('Manage'));
			cb.setCta();
			cb.onClick(async () => {
	...
	new Setting(containerEl)
		.setName(tr('Exclude pattern default settings'))
		.addButton((cb) => {
			cb.setButtonText(tr('Manage'));
			cb.setCta();
			cb.onClick(async () => {
	...
	new Setting(containerEl)
		.setName(tr('Add excluded folder'))
		.setClass('add-exclude-folder-item')
		.addButton((cb) => {
			cb.setIcon('plus');
			cb.setClass('add-exclude-folder');
			cb.setTooltip(tr('Add excluded folder'));
			cb.onClick(() => {
